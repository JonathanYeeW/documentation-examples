...
PART 1: THE SPREAD SELECTOR
  ...
  Step 2 — Handle selection → write to shared order state immediately
  Step 3 — Restore selection on remount (read from shared state on mount)

PART 2: SPREAD AVAILABILITY
  Step 1 — Fetch availability on every mount (never cached)
  Step 2 — Handle availability changes mid-session
  Step 3 — Operations override (manual, takes precedence over inventory)

PART 3: HANDOFF TO ASSEMBLY
  Step 1 — Include spreadType in order payload at confirmation
  Step 2 — Map selection to assembly instructions via order resolver
  Step 3 — Pass resolved instructions to spread applicator
```
