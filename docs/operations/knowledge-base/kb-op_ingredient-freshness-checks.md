...
PART 1: PRE-ASSEMBLY CHECK
  Step 1 — Run all ingredient checks in parallel
  Step 2 — Evaluate bread (bestByDate + moisture sensor)
  Step 3 — Evaluate spread (openedDate + per-spread freshness window)
  Step 4 — Evaluate jelly (openedDate + visual mold check)
  Step 5 — Fail fast on any failure → order fails, user notified

        ~~~~~~~~ assembly proceeds if all pass ~~~~~~~~

PART 2: MID-ASSEMBLY CHECK
  Step 1 — Check bread post-toasting (moisture sensor after cool-down)
  Step 2 — Skip for untoasted orders
  → Failure = TOAST_DRIED_BREAD failure code, order fails
```
...
This check runs immediately after ingredients are pulled from storage and staged at the prep station. If anything fails here, the order fails before a single step of assembly runs — no ingredients are wasted except a few seconds of check time.
